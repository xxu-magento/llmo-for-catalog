...
def _is_visible_text_element(element) -> bool:
    """
    Filter out script/style/meta/hidden/comment nodes.
    """
...
def _extract_visible_text(soup_or_element: BeautifulSoup) -> str:
    """
    Approximate what an LLM 'sees':
    - Only visible text nodes
    - Normalize whitespace
    - Drop empty / tiny fragments
    """
    texts = soup_or_element.find_all(string=True)
    chunks: List[str] = []
    for t in texts:
        if not _is_visible_text_element(t):
            continue
        cleaned = " ".join(t.strip().split())
        if not cleaned:
            continue
        # Optional heuristic: skip tiny nav fragments like '>' etc.
        if len(cleaned) < 2:
            continue
        chunks.append(cleaned)

    # Deduplicate adjacent duplicates a bit
    deduped: List[str] = []
    last = None
    for c in chunks:
        if c != last:
...
def _extract_agent_readable_content(soup: BeautifulSoup) -> str:
    """
    Try to approximate the main, LLM-facing content view,
    prioritizing central product areas before falling back
    to the whole page.
    """
    main_selectors = [
        "main",
    ...
    for sel in main_selectors:
        ...
        if region:
            return _extract_visible_text(region)

    # Fallback: whole page visible text
...
def scrape_adobestore_pdp(url: str) -> Dict[str, Any]:
    ...
    - Adds `agent_readable_text` approximating what an LLM sees after HTML → text.
    ...
    # --------------------------------
    result["agent_readable_text"] = _extract_agent_readable_content(soup)
    # Optional: keep a truncated raw HTML snapshot
    result["raw_html_snippet"] = resp.text[:20000]
...
if __name__ == "__main__":
    ...
    # Structured view
    pprint({k: v for k, v in data.items() if k not in ("agent_readable_text", "raw_html_snippet")})

    # print("\n=== AGENT READABLE TEXT (first 2000 chars) ===\n")
    print(data["agent_readable_text"])
